...
function high(x){
  ...
  // Iterate through alphabet to populate alphabet reference obj
  for (i = 0; i < alphaStr.length; i++) {
    alphaRef[alphaStr[i]] = values;
    values++;
  }

  // Add logic to iterate through letters of each word, to populate counterArr
  ...
  for (i=0; i<arr.length; i++) {
    ...
    for (j=0; j<arr[i].length; j++) {
      count += alphaRef[arr[i][j]];
    }
    counterArr.push(count);
  }

  // Iterate to compare point values, return final answer
  let maxCount = 0;
  let maxIndex = 0;

  for (i=0; i < counterArr.length; i++) {
    if (counterArr[i] > maxCount) {
      maxCount = counterArr[i];
      maxIndex = i;
    }
  }
  return arr[maxIndex]
